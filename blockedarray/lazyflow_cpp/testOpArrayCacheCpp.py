...
class TestOpArrayCache(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.data = (numpy.random.random(self.dataShape) * 100).astype(numpy.int64)
        ...
        print self.opProvider.Output.meta.dtype, "WTF?"
        
        opCache = OpArrayCacheCpp(graph=graph)
        opCache.Input.connect(opProvider.Output)
        
        print opCache.Input.meta.dtype, "WTF? 222"
    ...
    def testCacheAccess(self):
        ...
        #assert opProvider.accessCount == expectedAccessCount #FIXME
    ...
    def testFixAtCurrent(self):
        ...
        assert (data == 0).all(), "data.min() = %d, data.max() = %d" % (data.min(), data.max())
...
#
#        graph = Graph()
#        op = OpArrayCacheCpp(graph=graph)
